...
function stage({panel='',dialogue='',mode='',expression='friendly',step=1,turn='兩人一起',showCharacter=true,location=''}){
 const mine=dialogue.includes('data-speaker="我的心聲"')||dialogue.includes('data-speaker="'+actor()+'"');
 const detectiveScene=state.page==='chooseB';const p=(detectiveScene||(state.page==='practice'&&state.practiceRound===1))?1-state.actor:state.actor,c=avatar(p);
 const duo=mode==='story-screen',single=['choice-screen','practice-screen','reveal-screen'].includes(mode);
 const faceRight=['xiaoqing','ale','mimi'].includes(c.id);
 const selectedPortrait='<img class="inline-avatar" src="'+portrait(c)+'" alt="">';
 const figures=showCharacter?(duo?'<div class="vn-person player-person '+(mine?'speaking':'listening')+'"><img class="'+(faceRight?'':'mirror')+'" src="'+sprite(c)+'" alt="'+c.name+'"></div><div class="vn-person npc-person '+(!mine?'speaking':'listening')+'"><img src="assets/vn-xiaozhe-friendly.webp" alt="小哲拿著星星鉛筆盒"></div>':single?'<div class="vn-person solo-person"><img class="'+(faceRight?'':'mirror')+'" src="'+sprite(c)+'" alt="'+c.name+'"></div>':''):'';
 if(state.page==='reveal'){panel=panel.replace('<div class="card-owner">','<div class="card-owner"><img class="inline-avatar" src="'+portrait(avatar(state.actor))+'" alt="">');const marker='<div class="card-owner">'+esc(detective());panel=panel.replace(marker,'<div class="card-owner"><img class="inline-avatar" src="'+portrait(avatar(1-state.actor))+'" alt="">'+esc(detective()));}
 return '<section class="vn-stage v2-stage '+mode+' '+(mine||detectiveScene?'speaker-left':'speaker-right')+'" aria-label="第一關：勇敢開口"><img class="backdrop" src="assets/vn-classroom.webp" alt="陽光灑落的五年級教室"><div class="world-shade" aria-hidden="true"></div><div class="hud"><div class="location"><span class="chapter">01</span><span>彩虹小學 · 五年級教室<small>窗邊的新朋友</small></span></div><div class="turn-indicator">'+selectedPortrait+'<span>'+esc(turn)+'<small>'+esc(c.name)+' · '+step+' / 6</small></span></div></div>'+figures+panel+dialogue+'</section>';
}
...
function render(){
 ...
 case 'responseNext':{const r=STORY.next[state.next];html=stage({mode:'story-screen',expression:r.expression,step:4,panel:`<p class="scene-caption">${esc(r.gesture)}</p>`,dialogue:state.replyBeat===0?dialogue('小哲',r.reply,'responseNextFeedback','聽聽友情提示'):dialogue('友情小提示',r.feedback,r.kind==='retry'?'chooseNext':'noticeCharm',r.kind==='retry'?'換一句，再試試':'繼續故事')});break;}
