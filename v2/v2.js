...
function chapter(){return {title:'勇敢開口',location:'五年級教室',background:'vn-classroom.png'};}
function avatar(p){return CHARACTERS.find(c=>c.id===pair.ids[p])||CHARACTERS[p];}
function portrait(c){return `assets/characters/${c.id}-portrait.png`;}
function sprite(c){return `assets/characters/${c.id}-sprite.png`;}
...
async function saveMemory(){const status=document.querySelector('#memory-status');status.textContent='正在製作回憶圖片……';const b=document.querySelector('[data-action="v2_saveMemory"]');b.disabled=true;try{
 ...
 const images=await Promise.all(['assets/vn-classroom.png',...pair.ids.map((_,p)=>portrait(avatar(p)))].map(loadImage));
