...
function sprite(c,mood='neutral'){return `assets/characters/${c.id}-${mood==='happy'?'happy':'sprite'}.webp`;}
...
function memoryPage(){return `<section class="memory-world"><div class="memory-heading"><p class="kicker">共同回憶冊 · 第一頁</p><h1>從一句話，開始認識彼此。</h1><p>第一關完成 · 兩個人都練習了開口與回應。</p></div><article class="memory-book"><div class="memory-photo"><img class="memory-background" src="assets/vn-classroom.webp" alt="這次相遇的教室"><div class="photo-pair">${[0,1].map(p=>`${characterImage(avatar(p),p===0?"left":"right","happy")}`).join('')}</div><p>${esc(avatar(0).name)} ＆ ${esc(avatar(1).name)}<small>${esc(route().title)}</small></p></div><div class="memory-notes"><h2>今天，我們試過……</h2><div class="memory-entry">${roleChip(state.actor,'我的開場')}<p>${esc(personalize(STORY.opening[state.choice].text))}</p></div><div class="memory-entry">${roleChip(1-state.actor,'我喜歡的方式')}<p>${esc(STORY.preferences[state.preference])}</p></div><div class="memory-entry"><h3>我們一起接的下一句</h3><p>${esc(personalize(STORY.next[state.next].text))}</p></div><div class="takeaway"><h3>我們的共同約定</h3><p>${esc(route().invite)}</p><small>${esc(route().ending)}</small></div><div class="takeaway"><h3>帶回教室的一句話</h3><p>「嗨，我叫＿＿。你平常喜歡做什麼？」</p><small>想一想：下次，你想在什麼時候試著開口？</small></div></div></article><div class="memory-actions">${primary('前往後續任務地圖','v2_map')}<button class="secondary" data-action="v2_replay">交換任務，再玩一次</button><button class="secondary" data-action="v2_newPair">重新選角</button></div><p id="memory-status" role="status"></p><p class="memory-footnote">完成代表練習過，不代表交朋友能力的高低。重新整理會清除本次進度。</p></section>`;}
function freshPair(){reset();pair.ids=[null,null];pair.candidate=null;pair.selectPlayer=0;state.actor=0;campaign.level=1;campaign.completed=[];campaign.opening=null;campaign.decision=null;campaign.mini=[];goto('setup');}
function v2Action(b){
 ...
 else if(a==='v2_begin'){if(pair.ids.every(Boolean)&&pair.ids[0]!==pair.ids[1]){reset();configureStory();goto('intro');}}
 else if(a==='v2_hearOpening'){state.replyBeat=0;goto('responseA');}
 else if(a==='v2_hearNext'){state.replyBeat=0;goto('responseNext');}
 else if(a==='v2_replay'){reset();state.actor=1-state.actor;configureStory();goto('intro');}
...
async function saveMemory(){const status=document.querySelector('#memory-status');status.textContent='正在製作回憶圖片……';const b=document.querySelector('[data-action="v2_saveMemory"]');b.disabled=true;try{
 const canvas=document.createElement('canvas');canvas.width=1600;canvas.height=1400;const x=canvas.getContext('2d');
 ...
 x.fillStyle='#fff6df';x.fillRect(0,0,1600,1400);x.drawImage(images[0],0,0,1600,470);x.fillStyle='rgba(16,47,60,.84)';x.fillRect(0,0,1600,470);x.fillStyle='#ffe49a';x.font='bold 56px sans-serif';x.fillText('我們的第一頁友情回憶',70,100);x.fillStyle='white';x.font='30px sans-serif';x.fillText(route().title+' · 第一關完成',70,160);
 ...
 let y=535;const rows=[[actor()+'的開場',personalize(STORY.opening[state.choice].text)],[detective()+'喜歡的方式',STORY.preferences[state.preference]],['我們一起接的下一句',personalize(STORY.next[state.next].text)],['我們的共同約定',route().invite]];
 ...
 x.font='24px sans-serif';x.fillText('我們都試著開口，也都好好聽過對方。',80,1340);
